...
import { Mat, matMul, matTranspose, matIdentity } from "../src/matrix/index.js";

describe("Matrix", () => {
  it("should create matrix from 2D array", () => {
    const m = Mat.from2D([[1, 2], [3, 4]]);
  ...
  it("should multiply matrices", () => {
    const a = Mat.from2D([[1, 2], [3, 4]]);
    const b = Mat.from2D([[5, 6], [7, 8]]);
    const c = matMul(a, b);
  ...
  it("should transpose matrix", () => {
    const a = Mat.from2D([[1, 2], [3, 4], [5, 6]]);
