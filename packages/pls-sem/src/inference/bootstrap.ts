...
import { computeStructural, StructuralResult } from "../structural/index.js";
import { signFix, SignFixResult } from "./signfix.js";
...
export function bootstrap(
  ...
  for (const [key, dist] of loadingDists) {
    ...
    const orig = originalEstimate.loadings.get(cName)![
      model.constructs.find(c => c.name === cName)!.indicators.indexOf(indName)
    ];
  ...
  for (const [key, dist] of weightDists) {
    const [cName, indName] = key.split("::");
    const orig = originalEstimate.outerWeights.get(cName)![
      model.constructs.find(c => c.name === cName)!.indicators.indexOf(indName)
    ];
